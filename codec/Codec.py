...
c = 0
parola = list(map(str, input("Inserisci delle parole: ").replace("\'", " ").translate(str.maketrans('', '', string.punctuation)).rstrip("\n").lower().split()))
...
for i in range(0, len(numero)):
    ...
    with open("Italia2.txt") as file:
        for line in file:
            c = c + 1
            if numero[i] == 5 or numero[i] == 6:
                if c == int(numero[i]):
                    print(line.strip()+"\'", end='')
                    break
            elif numero[i].isnumeric() == True and int(numero[i]) == 0:
